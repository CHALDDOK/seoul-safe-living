...
구조: BASE_DATA(단지 마스터 85개) + 서울시 API(공고 매칭)
- 공고 제목으로 단지 매칭 → vacancy/notice 업데이트
- 전날 23:59까지 게시된 공고만 수집
- Nominatim geocoding (단지별 1회)
"""
...
CUTOFF_DATE = (NOW_KST - timedelta(days=1)).strftime('%Y-%m-%d')  # 전날 날짜
...
def geocode(address: str, gu: str):
...
def map_type(v):
    return {"1":"최초","2":"추가"}.get(str(v), str(v) if v else "")

def map_house(v):
    return {"1":"공공","2":"민간"}.get(str(v), str(v) if v else "")
...
def main():
    ...
    # 1) BASE_DATA 복사
    complexes = [dict(c) for c in BASE_DATA]
    notices_map = {c["id"]: [] for c in complexes}

    # 2) 역명 호선 정보 보강
    for c in complexes:
        raw = c.get("subway","")
        if raw:
            c["subwayFormatted"] = format_station(raw)
        else:
            c["subwayFormatted"] = ""
    ...
    try:
        ...
        for item in notices:
            ...
            # 공고 URL 구성
            url = f"{DETAIL_BASE}?nttId={ntt_id}&bbsId=BMSR00015" if ntt_id else \
                  f"{DETAIL_BASE}?boardId={board_id}&bbsId=BMSR00015" if board_id else ""

            matched = match_complex(title, complexes)
            if matched:
                cid = matched["id"]
                # 최근공고일 업데이트
                ...
                # 공고 카운트
                if ntype == "특별" or "특별" in title:
                    matched["special"] = matched.get("special",0) + 1
                elif ntype in ("최초","추가") or house == "민간":
                    matched["general"] = matched.get("general",0)  # 기존값 유지
    ...
    for c in complexes:
        lat, lng = geocode(c["name"], c["gu"])
        c["lat"] = lat
        c["lng"] = lng
        c["notices"] = notices_map.get(c["id"], [])

    # 5) KPI 계산 (민간임대 기준)
    active = [c for c in complexes if c["stage"] in ("입주","준공")]
    total_complexes = len(complexes)
    active_complexes = len([c for c in complexes if c["stage"] == "입주"])
    total_private = sum(c.get("private",0) for c in complexes)
    vacancy_units = sum(round(c.get("private",0) * c.get("vacancy",0)) for c in complexes)
    ...
    print(f"[INFO] data.json 저장 완료 (단지 {total_complexes}개, 공고 매칭 {sum(len(v) for v in notices_map.values())}건)")
