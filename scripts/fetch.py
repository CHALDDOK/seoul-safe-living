...
# ── 카카오 API 키 (GitHub Actions secret: KAKAO_MAP) ─────────────
...
DETAIL_MENU = "400008"   # menuNo 없으면 홈으로 리디렉션됨
...
# ── 단지별 좌표 오버라이드 (geocoding 오류 수동 보정) ────────────────
# key = pdfId (int): 지도 전용 단지 (PDF 연번 기준)
# key = BASE_DATA id (int): 테이블 단지 (BASE_DATA id 기준)
COMPLEX_COORDS_OVERRIDE = {
    86:  (37.6010, 127.0535),   # 성북구 석관동 173-1일원 (돌곶이역)
    122: (37.5660, 126.8058),   # 강서구 공항동 50-5 (송정역)
}
...
def _clean_address(addr: str) -> str:
    """지번주소 정제: '일원', '외N', '및 ...' 등 접미사 제거 → 첫 번지만 사용"""
    a = addr.strip()
    # "외 N필지", "외N", "일원", "및 ..." 등 제거
    a = re.sub(r'\s+(일원|외\s*\d+\s*필지?|외\s*\d+|및\s*.*)$', '', a)
    # 번지 범위 "N-M,P,Q" → "N-M" (첫 번지만)
    a = re.sub(r'(\d+-?\d*)[,\s]+\d.*$', r'\1', a)
    return a.strip()


def geocode_kakao(address: str) -> tuple | None:
    """카카오 로컬 API로 지번주소 → 위경도 변환"""
    ...
    try:
        resp = requests.get(
            ...
            params={'query': clean, 'analyze_type': 'exact'},
        ...
        if not docs:
            ...
            resp2 = requests.get(
                'https://dapi.kakao.com/v2/local/search/address.json',
                params={'query': clean, 'analyze_type': 'similar'},
                headers={'Authorization': f'KakaoAK {KAKAO_KEY}'},
                timeout=10,
            )
        ...
        if docs:
            ...
            lat = float(docs[0]['y'])
            # 서울 경계 검증
            if 37.40 <= lat <= 37.72 and 126.76 <= lng <= 127.20:
                return (lat, lng)
    except Exception as e:
        print(f'[WARN] 카카오 geocoding 실패 ({address}): {e}')
    return None


def geocode(address: str, gu: str, station: str | None = None) -> tuple:
    """주소 → (lat, lng): 카카오 API → STATION_COORDS → GU_CENTERS 순서로 시도"""
    # 1순위: 카카오 API (한국 지번주소 정확도 높음)
    if address:
        result = geocode_kakao(address)
        if result:
            return result
    # 2순위: 역 좌표 (STATION_COORDS)
    if station:
...
def fetch_notice_detail(ntt_id: str) -> dict:
    if not ntt_id:
        return {}
    url = f"{DETAIL_BASE}?nttId={ntt_id}&bbsId=BMSR00015&menuNo={DETAIL_MENU}"
    try:
        time.sleep(0.5)
...
def match_complex(title: str, data_list: list):
    """공고 제목에서 단지 매칭
    - 역명은 단어 변별에서 제외 (같은 역의 단지가 여러 개일 때 오매칭 방지)
    - 역방향 매칭: 공고 제목의 단어가 단지명에 포함되는지도 확인
    - 역명 포함 보너스로 지역 필터링
    """
    ...
    for d in data_list:
        score = 0
        n = d["name"].lower()
        subway_raw = re.sub(r'역$', '', d.get("subway","")).lower()

        if n in t or t in n:
            score = 100 + len(n)
        else:
            # 단지명 → 제목: 역명 단어는 제외 (공통이라 변별력 없음)
            words = [w for w in re.split(r'[\s\-_]+', n)
                     if len(w) >= 3 and w != subway_raw and w != subway_raw + '역']
            score += sum(10 for w in words if w in t)
            # 제목 → 단지명: 역방향 포함 확인 (2자 이상, 역명 제외)
            title_words = [w for w in re.split(r'[\s]+', t)
                           if len(w) >= 2 and w != subway_raw and w != subway_raw + '역']
            score += sum(8 for w in title_words if w in n)

        # 역명 포함 보너스 (지역 필터)
        if subway_raw and subway_raw in t:
            score += 50
...
def main():
    ...
    if os.path.exists(OUTPUT_PATH):
        try:
            ...
            for c in old.get('complexes', []):
                cid = str(c.get('id', ''))
                if cid and c.get('lat') and c.get('lng'):
                    existing_coords[cid] = (c['lat'], c['lng'])
    ...
    try:
        ...
        for item in notices:
            ...
            url = (f"{DETAIL_BASE}?nttId={ntt_id}&bbsId=BMSR00015&menuNo={DETAIL_MENU}" if ntt_id
                   else f"{DETAIL_BASE}?boardId={board_id}&bbsId=BMSR00015&menuNo={DETAIL_MENU}" if board_id else "")
    ...
    for c in all_complexes:
        # 오버라이드 우선 체크 (pdfId 또는 BASE_DATA id)
        c_id = c.get('id')
        if isinstance(c_id, str) and c_id.startswith('pdf_'):
            try:
                pdf_num = int(c_id.split('_')[1])
                if pdf_num in COMPLEX_COORDS_OVERRIDE:
                    c['lat'], c['lng'] = COMPLEX_COORDS_OVERRIDE[pdf_num]
                    continue
            except Exception:
                pass
        elif isinstance(c_id, int) and c_id in COMPLEX_COORDS_OVERRIDE:
            c['lat'], c['lng'] = COMPLEX_COORDS_OVERRIDE[c_id]
            continue

        # id 기반 캐시 조회
        geo_key = str(c.get('id', ''))
        if geo_key and geo_key in existing_coords:
    ...
    # 평균공실률: 100% 단지 제외 (최초모집 중 단지가 평균을 왜곡하지 않도록)
    avg_complexes    = [c for c in table_complexes if (c.get('vacancy') or 0) < 1.0]
    avg_private      = sum(c.get('private',0) for c in avg_complexes)
    avg_vacancy_u    = sum(round(c.get('private',0)*(c.get('vacancy') or 0)) for c in avg_complexes)
    avg_vr           = round(avg_vacancy_u/avg_private*100, 1) if avg_private else 0.0
